...
N = 4000; % Maximum number of nodes
...
node(1:N) = struct('x',ini_st*ones(1,2), 'min_clearance',ini_st ,'parent', ini_st,...
    'children',int16( ini_st*ones( ceil(N/3) ,1) ), 'children_num', 0,...
    'value', ini_value, 'removed', true);
...
radius = 0.1; %neiborhood and rewirimg radius
...
node(1).min_clearance = clearance(node(1).x.', obs_x, bound);
node(1).parent = 0;
node(1).value = node(1).min_clearance;
...
prop(1:num_props) = struct('x', ini_st*ones(1,2)); %, 'flag', false);
...
for ii=2:N
    ...
    dummy= 30 * gamma_star*(log(num_list+1)/(num_list+1))^(1/dim);
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%% Inserting a new node to the tree %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        ...
        while nbor_issue ~= 0
            ...
            [x, nearest] = sample_x_randomly(bound, node, In_list_ID, radius_min, r_attempt, obs_x, cell_size);
       

            [parent, min_clearance, value, nbor_issue] =...
                find_parent(x, node, r_attempt, obs_x, cell_size, bound, num_props, prop, In_list_ID, nearest, alpha);
        end
        

        % Insert the sampled node's information in node(ii) 
        node(ii).x = x;
        node(ii).min_clearance = min_clearance;
        node(ii).value = value;
        ...
        [nbors_rw_ID, ~, ~] = find_neighbors( x,  node,  r_attempt, In_list_ID, Is_only_neibor_ID, bound);
        ...
        for jj = 1:num_rw_nbor
            ...
            % check collision 
            issue_flag2(jj) = psuedo_obs_check_line2_oct(node(ii), node_jj, obs_x, cell_size,  num_props, prop);
        end
        ...
        if ~isempty(nbors_rw_no_issue_ID)
            ...
            cl_min1 = node(ii).min_clearance;
            value1 = node(ii).value;
            
            % total cost = Euclidean cost+ Info cost from node ii to k
            [val_new, min_cl_new] = dist_ig_mat2(node(ii).x.',  x_nbors_mat, cl_min1, value1, obs_x, bound, alpha);
            ...
            if ~isempty(rewired_better_ID)
                ...
                for k=1:length(rewired_better_ID)
                    ...
                    mat_parents_rewired_better = [node(old_parent_ID(k)).children];

                    % log in the new parent, the new value, and the new
                    % min_clearance 
                    node(rewired_better_ID(k)).parent = ii;
                    node(rewired_better_ID(k)).value = val_new(rewired_ID_in_nbors_rw(k));
                    node(rewired_better_ID(k)).min_clearance = min_cl_new(rewired_ID_in_nbors_rw(k));
                    ...
                    mat_parents_rewired_better([node(old_parent_ID(k)).children] == rewired_better_ID(k)) = int16( ini_st );
                    node(old_parent_ID(k)).children = mat_parents_rewired_better;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% updating the descendants of rewired nodes%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                    
                   ...
                   if length(descendants_k_ID)>=N
                    ...
                    for cnt=1:num_descendants_k
                        ...
                        cl_m = [node(m).min_clearance];
                        val_m = [node(m).value];

                        [value_mn, min_cl_mn] = dist_ig_mat( xm.', cl_m, val_m, xn', obs_x, bound, alpha);

                        node(n).value = value_mn;
                        node(n).min_clearance = min_cl_mn;
                    end
    ...
    if rem(ii,bnb_val) == 0
        if ~isempty(path) 
            [node, remove_ID{ii/bnb_val}] = branch_and_bound_2D(node,path,In_list_ID, ii, ini_st, dim, obs_x, bound, alpha); 
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
